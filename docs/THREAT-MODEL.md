...
to `git.johnwilger.com/jwilger/auto_review/ar-gateway`, attach Linux binary archives/checksums/signatures/SBOM/provenance metadata, and create Forgejo Releases only in `jwilger/auto_review`.
...
metadata, and uses `tea` to open the Forgejo release PR. The PR package publishing credential is distinct from the gateway bot credential and is used only after checkout/build steps have produced PR artifacts without `RELEASE_PUBLISH_TOKEN` in the environment. CI publishes PR Docker images under a package name distinct from final releases, publishes PR binary downloads as Forgejo generic packages, updates PR descriptions with package links using a managed PR body edit block, and deletes PR Docker and binary packages after the PR merges, including generic package deletion. Publish only runs for
release PRs merged into `main`, builds and verifies the Linux binary archives and metadata on the release runner before token-bearing publication, builds the release Docker image after the release PR merges to main, publishes only `git.johnwilger.com/jwilger/auto_review/ar-gateway` to the Forgejo package registry and creates the matching Forgejo Release entry with Linux binary archives and metadata from the merged release image archive, and refuses token-bearing publication when the merged release PR changed files outside expected root release metadata and intentional release tooling files: `Cargo.toml`, `Cargo.lock`, and `CHANGELOG.md`; `.forgejo/workflows/release-prepare.yml`, `.forgejo/workflows/release-publish.yml`, `scripts/release`, and `tests/release_tooling_test.sh` are also allowed for intentional release workflow/script/test changes.
...
to those Forgejo Releases; PR Docker/container package publish; PR Docker/container package delete;
generic package publish; generic package delete; and managed PR body/description edit.
