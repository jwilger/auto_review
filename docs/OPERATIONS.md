...
Configure the release publishing credential as Forgejo Actions secret `RELEASE_PUBLISH_TOKEN` owned by the same release bot named in `RELEASE_BOT_NAME`. Its release publishing PAT blast radius is to publish container images to `git.johnwilger.com/jwilger/auto_review/ar-gateway` and create Forgejo Releases only in `jwilger/auto_review`; the Forgejo Release scope includes Linux x86_64 and Linux aarch64 `auto-review` binary archive release assets plus SHA-256 checksums, signatures, SBOM, and provenance metadata. Set repository variable `RELEASE_AARCH64_NIX_BUILDER` to the pinned Nix remote-builder specification used for the Linux aarch64 package output; the publish workflow refuses to continue without it so a generic x86_64 Docker runner does not silently skip or fake the aarch64 archive. Release preparation builds the release candidate image with `nix build .#ar-gateway-image` after the signed release metadata commit, publishes release candidate image tags for the release PR head SHA and `$RELEASE_VERSION-rc.$GITHUB_RUN_NUMBER`, creates or updates a prerelease Forgejo Release entry for the `$RELEASE_VERSION-rc.$GITHUB_RUN_NUMBER` tag, and includes those candidate refs in the release PR description so operators can test the exact release candidate image tag before merge. Final publication builds and verifies the Linux binary archives first, promotes the release candidate image to the release version and `latest` tags instead of rebuilding it, publishes the final binary release assets, and includes verification commands in the release notes:
