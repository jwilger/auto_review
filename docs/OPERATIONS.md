...
Release automation uses two Forgejo Actions secrets with separate blast radii.
Configure the release preparation credential as Forgejo Actions secret `FORGEJO_RELEASE_PREPARE_TOKEN`; its release preparation PAT blast radius is to
prepare release PR branches and release PRs only in `jwilger/auto_review`.
