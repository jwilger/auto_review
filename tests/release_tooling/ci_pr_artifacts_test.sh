...
if not any(candidate in workflow for candidate in rc_image_candidates):
    errors.append('CI workflow must publish release PR Docker images under a package name distinct from final releases, such as ar-gateway-rc or ar-gateway-pr')
if re.search(r'docker://git\.johnwilger\.com/jwilger/auto_review/ar-gateway:(?:pr-|rc-|\$\{?PR_)', workflow):
    errors.append('CI workflow must not publish release PR Docker images as tags under the final ar-gateway package name')
...
release_pr_publication_steps = [
    ...
    if 'ar-gateway-pr' in step.group('body') and ('skopeo copy' in step.group('body') or 'Docker image:' in step.group('body'))
]
release_pr_publication_text = '\n'.join(release_pr_publication_steps)
if not re.search(r'ar-gateway-pr:release-v(?:\$\{?RELEASE_VERSION\}?)[^\s"\']*(?:\$\{?PR_HEAD_SHA\}?|github\.event\.pull_request\.head\.sha)', release_pr_publication_text):
    errors.append('CI release PR Docker image tag must bind release-v$RELEASE_VERSION to the immutable PR head SHA')
if re.search(r'ar-gateway-pr:release-v\$\{?RELEASE_VERSION\}?(?:["\'\s]|$)', release_pr_publication_text):
    errors.append('CI release PR Docker image tag must not be the mutable version-only release-v$RELEASE_VERSION tag')
