...
PY
  ...
  assert_file_not_contains "$publish_workflow" 'nix build .#ar-gateway-image' "publish workflow does not rebuild the release Docker image after the release PR merges to main"
  assert_file_contains "$publish_workflow" '$RELEASE_MERGE_SHA' "publish workflow has the release merge SHA available for release PR Docker image promotion"
  assert_file_not_contains "$publish_workflow" 'git.johnwilger.com/jwilger/auto_review/ar-gateway-rc' "publish workflow promotes the release PR Docker image from the final repository digest instead of an RC-only image repository"
  assert_file_not_contains "$publish_workflow" 'git.johnwilger.com/jwilger/auto_review/ar-gateway-pr' "publish workflow promotes the release PR Docker image from the final repository digest instead of a PR-only image repository"
  assert_file_not_contains "$publish_workflow" 'git.johnwilger.com/jwilger/auto_review/pr-ar-gateway' "publish workflow promotes the release PR Docker image from the final repository digest instead of an alternate PR-only image repository"
