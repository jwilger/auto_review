...
- Removed the runtime linter sandbox image (`deploy/Dockerfile.sandbox`) and
  unhooked sandbox selection from gateway/orchestrator review jobs.
- Added a Nix-built `.#ar-gateway-image` OCI image and
  `nix run .#dev-gateway-container` watcher for local containerized development;
  the old bacon run/restart job is retired in favor of rebuilding and relaunching
  the deploy-shaped image.
...
- Re-scoped the remaining gateway/orchestrator workspace paths after linter
  ...
  `AR_SANDBOX_IMAGE` is no longer a gateway startup requirement, and `/info` no
  longer exposes a normal-runtime sandbox field. Closes #46.
