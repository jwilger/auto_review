...
Run the narrow focused command, capture the exact RED output, and explain why the single failure is expected. Treat compiler errors as valid RED when the test intentionally pressures a missing API or type. Fix only test misuse of existing code; do not edit production code. For Rust, tests may live in `tests/` integration/acceptance files or inside source files under `#[cfg(test)] mod tests`; when editing a source file, change only the in-source test module and leave production items untouched.
