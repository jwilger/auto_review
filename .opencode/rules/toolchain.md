...
Focused checks are `just fmt`, `just clippy`, `just test`, `just opencode-test` for `.opencode/` harness/plugin changes, `just deny`, and `just build`. Use `just ci` for the aggregate routine gate.
